...
// 🔐 Load and validate secret (must exist and be >= 32 bytes when encoded)
const rawSecret = process.env.APP_SESSION_SECRET;
if (!rawSecret) {
  // eslint-disable-next-line no-console
  console.error('[session/issue] APP_SESSION_SECRET is not set');
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!rawSecret || secret.byteLength < 32) {
    return res
      .status(500)
      .json({ ok: false, error: 'invalid_server_secret', message: 'APP_SESSION_SECRET must be at least 32 bytes.' });
  }
  ...
  try {
    ...
    // Resolve environment and credentials from idToken custom claims
    ...
    if (!apiKey || !apiSecret) {
      return res.status(400).json({ ok: false, error: 'missing_creds', message: 'Custom claims username/password absent.' });
    }

    // Build encrypted session (JWE)
    ...
    return res.status(400).json({ ok: false, error: 'issue_failed', message: e?.message || 'decode_or_encrypt_failed' });
  }
}