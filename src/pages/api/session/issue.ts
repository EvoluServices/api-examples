...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const secretRaw = await getSessionSecret();
    ...
    res.setHeader('Set-Cookie', serialize('app-session', jwe, {
      httpOnly: true,
      sameSite: 'lax',
    ...
    console.error('[session/issue] runtime secret error:', e?.message);
    res.status(500).json({ ok: false, error: 'invalid_server_secret', message: e?.message });
  }
}